...
# Check if running in systemd service (required for resource guardrails)
# Must be after load_dotenv() to allow AUTOCODER_SYSTEMD_SERVICE override for testing
import sys
import os
from pathlib import Path
...
if not _check_systemd_cgroup():
    ...
    print()
    print("To bypass this check (testing only):", file=sys.stderr)
    print("  AUTOCODER_ALLOW_MANUAL=1 python autonomous_agent_demo.py ...", file=sys.stderr)
    print("=" * 70, file=sys.stderr)
    sys.exit(1)
